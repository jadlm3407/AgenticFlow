...
async function listActions(storage, auth, vargs) {
    ...
    const applyTimestampFilters = (q) => {
        if (!timeFilterRequested)
            return;
        q.whereNotNull('created_at');
        if (createdAtFrom)
            q.where('created_at', '>=', storage.validateDateForWhere(createdAtFrom));
        if (createdAtTo)
            q.where('created_at', '<', storage.validateDateForWhere(createdAtTo));
    };
