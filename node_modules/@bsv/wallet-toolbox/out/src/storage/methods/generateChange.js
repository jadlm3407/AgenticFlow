...
exports.maxChangeOutputsPerTransaction = exports.maxPossibleSatoshis = void 0;
 ...
 * rather than 1.
 */
exports.maxChangeOutputsPerTransaction = 8;
/**
 * Simplifications:
 ...
 * @returns
 */
async function generateChangeSdk(params, allocateChangeInput, releaseChangeInput, logger) {
    var _a;
    ...
    try {
         ...
         */
        const minSpendTxSize = (0, utils_1.transactionSize)([params.changeUnlockingScriptLength], [params.changeLockingScriptLength]);
         ...
         */
        const maxChangeOutputs = (_a = params.maxChangeOutputs) !== null && _a !== void 0 ? _a : exports.maxChangeOutputsPerTransaction;
        ...
        const addOutputToBalanceNewInput = () => {
            ...
            if (r.changeOutputs.length >= maxChangeOutputs)
                return false;
        ...
        while (r.changeOutputs.length < maxChangeOutputs &&
         ...
         */
        if (feeExcess() < 0) {
            const werr = new WERR_errors_1.WERR_INSUFFICIENT_FUNDS(spending() + feeTarget(), -feeExcessNow);
            logger === null || logger === void 0 ? void 0 : logger.error(`throwing WERR_INSUFFICIENT_FUNDS moreSatoshisNeeded ${werr.moreSatoshisNeeded}`);
            await releaseAllocatedChangeInputs();
            throw werr;
        }
        /**
...
function validateGenerateChangeSdkParams(params) {
    ...
    params.fixedInputs.forEach((x, i) => {
        sdk_1.Validation.validateSatoshis(x.satoshis, `fixedInputs[${i}].satoshis`);
        sdk_1.Validation.validateInteger(x.unlockingScriptLength, `fixedInputs[${i}].unlockingScriptLength`, undefined, 0);
    });
    ...
    params.fixedOutputs.forEach((x, i) => {
        sdk_1.Validation.validateSatoshis(x.satoshis, `fixedOutputs[${i}].satoshis`);
        sdk_1.Validation.validateInteger(x.lockingScriptLength, `fixedOutputs[${i}].lockingScriptLength`, undefined, 0);
    ...
    sdk_1.Validation.validateOptionalInteger(params.targetNetCount, `targetNetCount`);
    sdk_1.Validation.validateSatoshis(params.changeFirstSatoshis, 'changeFirstSatoshis', 1);
    sdk_1.Validation.validateSatoshis(params.changeInitialSatoshis, 'changeInitialSatoshis', 1);
    sdk_1.Validation.validateInteger(params.changeLockingScriptLength, `changeLockingScriptLength`);
    sdk_1.Validation.validateInteger(params.changeUnlockingScriptLength, `changeUnlockingScriptLength`);
    return r;
}
