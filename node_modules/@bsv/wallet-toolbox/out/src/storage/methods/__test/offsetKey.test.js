...
async function createRedemptionTestContext() {
    ...
    const setup = await TestUtilsWalletStorage_1._tu.createTestWalletWithStorageClient({
        chain: 'main',
...
async function tryAddCommissionInput(comm, storage, beef, chainTracker, inputs, comms) {
...
async function collectCommissionInputs(storage, fca, beef, chainTracker) {
    const comms = [];
    const inputs = [];
...
async function signAndBroadcastRedemption(setup, storage, env, comms, inputs, beef, chainTracker) {
    var _a;
    ...
    for (let vin = 0; vin < comms.length; vin++) {
        const { hashedSecret } = (0, offsetKey_1.keyOffsetToHashedSecret)(pub, comms[vin].keyOffset);
        tx.inputs[vin].unlockingScriptTemplate = p2pkh.unlock(new sdk_1.PrivateKey(priv.add(hashedSecret).mod(curve.n)), 'all', false);
    }
    await tx.sign();
    const spends = {};
    for (let vin = 0; vin < comms.length; vin++) {
 ...
 */
async function runRedemptionLoop(storage, setup, env, fca) {
    for (;;) {
