...
function toAdminStatsLog(stats) {
    var _a, _b;
    const row = (label, day, week, month, total) => `  ${alignLeft(label, 13)} ${alignRight(day !== null && day !== void 0 ? day : '', 18)} ${alignRight(week !== null && week !== void 0 ? week : '', 18)} ${alignRight(month !== null && month !== void 0 ? month : '', 18)} ${alignRight(total !== null && total !== void 0 ? total : '', 18)}\n`;
    let log = `StorageAdminStats: ${(_a = stats.when) !== null && _a !== void 0 ? _a : ''} ${(_b = stats.requestedBy) !== null && _b !== void 0 ? _b : ''}\n`;
    ...
    log += row('change sats', formatSatoshis(stats.satoshisDefaultDay), formatSatoshis(stats.satoshisDefaultWeek), formatSatoshis(stats.satoshisDefaultMonth), formatSatoshis(stats.satoshisDefaultTotal));
    log += row('other sats', formatSatoshis(stats.satoshisOtherDay), formatSatoshis(stats.satoshisOtherWeek), formatSatoshis(stats.satoshisOtherMonth), formatSatoshis(stats.satoshisOtherTotal));
...
function formatSatoshis(value) {
    ...
    return Format_1.Format.satoshis(Number.isFinite(n) ? n : 0);
}
    ...
    return {
        ...
        inputs: tx.inputs.map((input, vin) => {
            ...
            return ({
                ...
                unlockingScriptLength: (_b = (_a = input.unlockingScript) === null || _a === void 0 ? void 0 : _a.toBinary) === null || _b === void 0 ? void 0 : _b.call(_a).length
            });
        }),
        outputs: tx.outputs.map((output, vout) => {
            ...
            return ({
                ...
                lockingScriptHex: (_b = (_a = output.lockingScript) === null || _a === void 0 ? void 0 : _a.toHex) === null || _b === void 0 ? void 0 : _b.call(_a)
            });
        })
    };
}
async function getStorage(context) {
    var _a;
    const storage = (_a = context.daemon.setup) === null || _a === void 0 ? void 0 : _a.storageProvider;
    if (!storage)
        throw new Error('Monitor storage provider is not available.');
    return storage;
}
async function getKnex(context) {
    const storage = await getStorage(context);
    const knex = storage.knex;
    if (!knex)
        throw new Error('Joined admin review requires StorageKnex / MySQL.');
    return knex;
}
async function queryReqReview(context, query) {
    var _a;
    ...
    if (txid)
        base.andWhere('r.txid', txid);
    if (batch)
        base.andWhere('r.batch', batch);
    if (userId)
        base.andWhere('t.userId', userId);
    const totalResult = (await base.clone().count({ count: '*' }));
    const total = Number(((_a = totalResult[0]) === null || _a === void 0 ? void 0 : _a.count) || 0);
...
function getReviewUtxosTask(context) {
    var _a;
    const monitor = (_a = context.daemon.setup) === null || _a === void 0 ? void 0 : _a.monitor;
    if (!monitor)
        throw new Error('Monitor is not available.');
    const task = [...monitor._tasks, ...monitor._otherTasks].find(item => item.name === 'ReviewUtxos');
    if (!(task === null || task === void 0 ? void 0 : task.reviewByIdentityKey)) {
...
async function queryUsers(context, query) {
    const storage = await getStorage(context);
    ...
    const filtered = search
        ? users.filter(user => user.identityKey.includes(search) || (user.userId !== undefined && String(user.userId).includes(search)))
        : users;
    return {
...
async function resolveIdentityKeyFromInput(context, userInput) {
    const value = userInput.trim();
    if (!value)
        throw new Error('identityKey or userId is required.');
    if (value.length < 64) {
        const storage = await getStorage(context);
        ...
        if (!(user === null || user === void 0 ? void 0 : user.identityKey))
            throw new Error(`User ${userId} was not found.`);
        return user.identityKey;
    }
    return value;
}
async function queryMonitorCallHistory(context, query) {
    const storage = await getStorage(context);
    const limit = Math.min(Math.max(asNumber(query.limit, 120), 1), 120);
    const offset = Math.max(asNumber(query.offset, 0), 0);
    const selectedId = asNumber(query.selectedId, -1);
    const partial = { event: 'MonitorCallHistory' };
...
async function reviewUtxosByIdentityKey(context, requestedBy, userInput, mode) {
    const storage = await getStorage(context);
...
async function getReqDetail(context, provenTxReqId) {
    const storage = await getStorage(context);
    const req = (await storage.findProvenTxReqs({ partial: { provenTxReqId } }))[0];
    if (!req)
        throw new Error(`ProvenTxReq ${provenTxReqId} not found`);
    const txs = req.txid
...
async function rebroadcastReq(context, provenTxReqId, provider, requestedBy) {
    var _a;
    const storage = await getStorage(context);
    const req = (await storage.findProvenTxReqs({ partial: { provenTxReqId } }))[0];
    if (!(req === null || req === void 0 ? void 0 : req.rawTx))
        throw new Error(`ProvenTxReq ${provenTxReqId} is missing rawTx`);
    const services = (_a = context.daemon.setup) === null || _a === void 0 ? void 0 : _a.services;
    if (!(services instanceof Services_1.Services)) {
...
class AdminServer {
    ...
    getSdkBundlePath() {
        ...
        return path_1.default.resolve(path_1.default.dirname(sdkMainPath), '../umd/bundle.js');
    }
    setupRoutes() {
        ...
        this.app.get('/admin/assets/bsv-sdk.js', (_req, res) => {
            res.type('application/javascript').sendFile(this.getSdkBundlePath());
        });
        this.app.get('/admin', (_req, res) => {
            res.type('text/html').send((0, adminUi_1.renderAdminPage)());
        });
        ...
        this.app.get('/admin/api/stats', async (req, res) => {
        ...
        this.app.get('/admin/api/events', async (req, res) => {
            const storage = await getStorage(this.context);
        ...
        this.app.get('/admin/api/tasks', async (_req, res) => {
            var _a;
            const monitor = (_a = this.context.daemon.setup) === null || _a === void 0 ? void 0 : _a.monitor;
            if (!monitor)
                throw new Error('Monitor is not available.');
            res.json({
                tasks: [
        ...
        this.app.get('/admin/api/users', async (req, res) => {
            res.json(await queryUsers(this.context, req.query || {}));
        });
        this.app.get('/admin/api/call-history', async (req, res) => {
            res.json(await queryMonitorCallHistory(this.context, req.query || {}));
        });
        this.app.post('/admin/api/tasks/:name/run', async (req, res) => {
            var _a;
            const monitor = (_a = this.context.daemon.setup) === null || _a === void 0 ? void 0 : _a.monitor;
            const storage = await getStorage(this.context);
            if (!monitor)
        ...
        this.app.post('/admin/api/review-utxos', async (req, res) => {
            var _a, _b;
            const userInput = typeof ((_a = req.body) === null || _a === void 0 ? void 0 : _a.identityKey) === 'string' ? req.body.identityKey.trim() : '';
            if (!userInput)
                throw new Error('identityKey or userId is required.');
            const mode = normalizeReviewMode((_b = req.body) === null || _b === void 0 ? void 0 : _b.mode);
            res.json(await reviewUtxosByIdentityKey(this.context, req.auth.identityKey, userInput, mode));
        });
        this.app.get('/admin/api/proven-tx-reqs/review', async (req, res) => {
            res.json(await queryReqReview(this.context, req.query || {}));
        });
        this.app.get('/admin/api/proven-tx-reqs/:id', async (req, res) => {
            res.json(await getReqDetail(this.context, asNumber(req.params.id, -1)));
        });
        this.app.get('/admin/api/proven-tx-reqs/:id/decode', async (req, res) => {
        ...
        this.app.post('/admin/api/proven-tx-reqs/:id/rebroadcast', async (req, res) => {
            var _a;
            const provider = typeof ((_a = req.body) === null || _a === void 0 ? void 0 : _a.provider) === 'string' ? req.body.provider : 'taal';
    ...
    requireAdmin(req, _res, next) {
        var _a;
        const identityKey = (_a = req.auth) === null || _a === void 0 ? void 0 : _a.identityKey;
    ...
    start() {
        this.server = this.app.listen(this.context.config.adminPort, this.context.config.adminHost, () => {
            console.log(`Monitor admin listening at http://${this.context.config.adminHost}:${this.context.config.adminPort}/admin`);
        });
    }
    async close() {
        if (!this.server)
