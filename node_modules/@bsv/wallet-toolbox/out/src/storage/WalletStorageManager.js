...
class WalletStorageManager {
    ...
    async internalizeAction(args) {
        sdk_1.Validation.validateInternalizeActionArgs(args);
        return await this.runAsWriter(async (writer) => {
            const auth = await this.getAuth(true);
    ...
    async relinquishCertificate(args) {
        sdk_1.Validation.validateRelinquishCertificateArgs(args);
        return await this.runAsWriter(async (writer) => {
            const auth = await this.getAuth(true);
    ...
    async relinquishOutput(args) {
        sdk_1.Validation.validateRelinquishOutputArgs(args);
        return await this.runAsWriter(async (writer) => {
            const auth = await this.getAuth(true);
     ...
     */
    async reproveHeightMerkleRoot(height, staleMerkleRoot) {
        const r = { log: '', updated: [], unchanged: [], unavailable: [] };
        let ptxs = [];
        await this.runAsStorageProvider(async (sp) => {
     ...
     * @returns
     */
    async reproveProven(ptx, noUpdate) {
        const r = { log: '', updated: undefined, unchanged: false, unavailable: false };
