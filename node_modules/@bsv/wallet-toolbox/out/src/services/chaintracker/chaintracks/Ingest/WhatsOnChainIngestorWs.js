...
exports.WocHeadersBulkListener = WocHeadersBulkListener;
exports.WocHeadersBulkListener_test = WocHeadersBulkListener_test;
exports.WocHeadersLiveListener = WocHeadersLiveListener;
exports.WocHeadersLiveListener_test = WocHeadersLiveListener_test;
const ws_1 = __importDefault(require("ws"));
const WhatsOnChain_1 = require("../../../providers/WhatsOnChain");
const utilityHelpers_1 = require("../../../../utility/utilityHelpers");
async function getWhatsOnChainTipHeight(chain = 'main', apiKey) {
    const config = apiKey ? { apiKey } : {};
    const woc = new WhatsOnChain_1.WhatsOnChain(chain, config);
 ...
 */
async function WocHeadersBulkListener(fromHeight, toHeight, enqueue, error, stop, chain, logger = () => { }, idleWait = 5000) {
            ...
            case 0:
                ...
                if (!done) {
                    ...
                    if (json === '{}') {
                        ...
                        if (wocHeader) {
                            const header = (0, WhatsOnChain_1.convertWocToBlockHeaderHex)(wocHeader);
                            enqueue(header);
                            count++;
    ...
    const ws = new ws_1.default(webSocketUrl);
    ...
    while (!done) {
        ...
        await (0, utilityHelpers_1.wait)(idleWait);
 ...
 */
async function WocHeadersBulkListener_test() {
    try {
        const chains = ['main', 'test'];
        // eslint-disable-next-line prefer-const
        const stop = { stop: undefined };
        for (const chain of chains) {
            const apiKey = undefined;
            const tipHeight = await getWhatsOnChainTipHeight(chain, apiKey);
            let errors = 0;
            while (errors === 0 &&
                !(await WocHeadersBulkListener(tipHeight - 3, tipHeight, header => console.log(JSON.stringify(header)), (code, message) => {
                    errors++;
                    console.log(`code ${code} message ${message}`);
                    return true;
                }, stop, chain, console.log.bind(console), 5000))) {
 ...
 */
async function WocHeadersLiveListener(enqueue, error, stop, chain, logger, idleWait = 100000) {
    let count = 0;
    let ok = false;
    let done = false;
    let wsIsOpen = false;
            ...
            case 0:
                ...
                if (!done) {
                    if (json === '{}') {
                        ...
                        const wocHeader = ((_a = data.pub) === null || _a === void 0 ? void 0 : _a.data) || ((_b = data.data) === null || _b === void 0 ? void 0 : _b.data);
                        if (wocHeader) {
                            const header = (0, WhatsOnChain_1.convertWocToBlockHeaderHex)(wocHeader);
                            enqueue(header);
                            count++;
    ...
    while (!done) {
        await (0, utilityHelpers_1.wait)(waitMsecs);
...
async function WocHeadersLiveListener_test() {
    try {
        const chain = 'main';
        const stop = { stop: undefined };
        let errors = 0;
        while (errors === 0 &&
            !(await WocHeadersLiveListener(header => console.log(JSON.stringify(header)), (code, message) => {
                errors++;
                console.log(`code ${code} message ${message}`);
                return true;
            }, stop, chain, console.log.bind(console), 100000))) {
            console.log('restarting WoC addHeaders');
        }
    }
    catch (err) {
 ...
 */
//# sourceMappingURL=WhatsOnChainIngestorWs.js.map