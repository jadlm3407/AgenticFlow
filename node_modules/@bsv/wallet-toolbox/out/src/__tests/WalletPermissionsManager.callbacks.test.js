...
describe('WalletPermissionsManager - Callbacks & Event Handling', () => {
    ...
    describe('grantGroupedPermission error handling', () => {
        it('should reject pending promises when grantGroupedPermission throws a validation error', async () => {
            ...
            const groupedCb = globals_1.jest.fn(() => { });
            ...
            const originalRequest = {
                ...
                permissions: {
                    protocolPermissions: [
                        { protocolID: [1, 'requested-protocol'], counterparty: 'self', description: 'test' }
                    ]
                }
            };
            ...
            const pendingPromise = new Promise((resolve, reject) => {
                ;
                manager.activeRequests.set(requestID, {
            ...
            const grantPromise = manager.grantGroupedPermission({
                ...
                granted: {
                    protocolPermissions: [
                        { protocolID: [1, 'DIFFERENT-protocol'], counterparty: 'self', description: 'test' }
                    ]
                }
            });
        ...
        it('should clean up activeRequests even when grantGroupedPermission throws', async () => {
            ...
            expect(manager.activeRequests.has(requestID)).toBe(true);
            // Grant with mismatched spending authorization to trigger an error
            await expect(manager.grantGroupedPermission({
                requestID,
                granted: {
        ...
        it('should resolve pending promises when grantGroupedPermission succeeds', async () => {
            ...
            globals_1.jest.spyOn(manager, 'createPermissionOnChain').mockResolvedValue(undefined);
            const requestID = 'group:success-test.com';
            const requestedPermissions = {
                protocolPermissions: [
                    { protocolID: [1, 'test-proto'], counterparty: 'self', description: 'Test' }
                ]
            };
