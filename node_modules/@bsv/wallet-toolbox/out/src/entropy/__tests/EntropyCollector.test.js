...
const EntropyCollector_1 = require("../EntropyCollector");
describe('EntropyCollector', () => {
    describe('Configuration', () => {
        it('should use default configuration', () => {
            const collector = new EntropyCollector_1.EntropyCollector();
    ...
    describe('Sample Collection', () => {
        it('should collect mouse samples', () => {
            const collector = new EntropyCollector_1.EntropyCollector({ targetSamples: 10, minSampleInterval: 0 });
        ...
        it('should reject samples that are too close together', () => {
            const collector = new EntropyCollector_1.EntropyCollector({ targetSamples: 10, minSampleInterval: 1000 });
        ...
        it('should cap percent at 100', () => {
            const collector = new EntropyCollector_1.EntropyCollector({ targetSamples: 5, minSampleInterval: 0 });
            for (let i = 0; i < 10; i++) {
                collector.addMouseSample(i * 10, i * 20);
            }
            const progress = collector.getProgress();
    ...
    describe('Completion', () => {
        it('should report incomplete when not enough samples', () => {
            const collector = new EntropyCollector_1.EntropyCollector({ targetSamples: 100, minSampleInterval: 0 });
            for (let i = 0; i < 50; i++) {
    ...
    describe('Entropy Estimation', () => {
        ...
        it('should cap entropy estimate at 256 bits', () => {
            const collector = new EntropyCollector_1.EntropyCollector({ targetSamples: 1000, minSampleInterval: 0 });
            for (let i = 0; i < 500; i++) {
                collector.addMouseSample(i, i);
            }
            const bits = collector.estimateEntropyBits();
...
//# sourceMappingURL=EntropyCollector.test.js.map